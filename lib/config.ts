...
export const getChainId = (chain: string): number => {
  ...
  // 首先检查是否是KyberSwap链名称
  const kyberChain = kyberSwapChains.find(c => 
    c.displayName.toUpperCase().replace(/\s+/g, '_') === chain || 
    c.chainName.toUpperCase() === chain
  );
...
export const initializeTokensCache = async (): Promise<void> => {
  ...
  try {
    ...
    console.log(`Fetching tokens for ${sortedChains.length} chains with max concurrency ${config.maxConcurrentRequests}`);
...
// 调试函数：获取特定链上的所有代币
export const getDebugTokensForChain = (chainIdOrName: string | number): {
...
export const forceRefreshKyberSwapCache = async (): Promise<boolean> => {
  try {
    ...
    // 只加载优先链的代币数据
    const priorityChains = chains.filter(chain => 
