import type { Strategy, PriceHistoryRecord, TradeExecution, ChainConfig, TokenConfig } from "./types"
...
// 更新策略自动交易设置
export const updateStrategyAutoTrade = (id: string, autoTrade: boolean, minProfitPercentage?: string): void => {
  const strategies = getStrategies()
  const index = strategies.findIndex((s) => s.id === id)
  ...
  if (index !== -1) {
    strategies[index] = {
      ...strategies[index],
      autoTrade,
      minProfitPercentage: minProfitPercentage || strategies[index].minProfitPercentage || "1.0",
      updatedAt: Date.now(),
    }
    saveStrategies(strategies)
  }
...
export const addPriceHistoryRecord = (record: PriceHistoryRecord): void => {
  ...
  const filteredHistory = history.filter(
    (r) =>
      r.strategyId !== record.strategyId || history.filter((h) => h.strategyId === record.strategyId).indexOf(r) < 9,
  )

  filteredHistory.push(record)
  savePriceHistory(filteredHistory)
}

// 获取特定策略的价格历史记录
export const getStrategyPriceHistory = (strategyId: string): PriceHistoryRecord[] => {
  const history = getPriceHistory()
  return history.filter((record) => record.strategyId === strategyId).sort((a, b) => b.timestamp - a.timestamp)
}
...
// 更新交易执行记录
export const updateTradeExecution = (updatedExecution: TradeExecution): void => {
  const executions = getTradeExecutions()
...
// 获取特定策略的交易执行记录
export const getStrategyTradeExecutions = (strategyId: string): TradeExecution[] => {
  const executions = getTradeExecutions()
  return executions.filter((execution) => execution.strategyId === strategyId).sort((a, b) => b.timestamp - a.timestamp)
}
...
export const exportStrategies = (ids?: string[]): string => {
  const strategies = getStrategies()
  const strategiesToExport = ids ? strategies.filter((s) => ids.includes(s.id)) : strategies

  return JSON.stringify(strategiesToExport)
}

// 添加导入策略功能
export const importStrategies = (jsonData: string): { success: boolean; count: number; error?: string } => {
  try {
    ...
    const validStrategies = strategies.filter(
      (s) => s.id && s.name && s.sourceChain && s.sourceToken && s.targetChain && s.targetToken && s.initialAmount,
    )
