...
const fetch1InchQuote = async (
  ...
  try {
    ...
    const quote: PriceQuote = {
      ...
      amountInUsd: data.prices?.usd?.fromToken || "0",
      amountOutUsd: data.prices?.usd?.toToken || "0",
      gasSpent: data.gas,
...
const fetchSushiQuote = async (
  ...
  try {
    ...
    if (data.tokens && Array.isArray(data.tokens) && data.tokens.length > 0) {
      // 使用API返回的tokens
      quote.tokens = data.tokens;

      // 查找tokenIn和tokenOut在tokens数组中的匹配项
      ...
      if (tokenInInfo) {
        const amountInDecimal = convertWeiToDecimal(
          quote.amountIn,
          tokenInInfo.decimals
        );
        quote.amountInUsd = Number(amountInDecimal).toString();
      }

      if (tokenOutInfo) {
        const amountOutDecimal = convertWeiToDecimal(
          quote.assumedAmountOut,
          tokenOutInfo.decimals
        );
        quote.amountOutUsd = Number(amountOutDecimal).toString();
      }
    }

    console.log("Final SushiQuote:", quote);
    return quote;
  } catch (error) {
...
const fetchUniswapQuote = async (
  ...
  try {
    ...
    const data: PriceQuote = {
      ...
      amountInUsd: (Number.parseFloat(amount) * 100).toString(), // 模拟USD价格
      amountOutUsd: (Number.parseFloat(amount) * swapPrice * 100).toString(), // 模拟USD价格
...
const fetchParaSwapQuote = async (
  ...
  try {
    ...
    const data: PriceQuote = {
      ...
      amountIn: amount,
      assumedAmountOut: (Number.parseFloat(amount) * swapPrice).toString(),
      amountInUsd: (Number.parseFloat(amount) * 120).toString(), // 模拟USD价格
      amountOutUsd: (Number.parseFloat(amount) * swapPrice * 120).toString(), // 模拟USD价格
...
const fetchKyberSwapQuote = async (
  ...
  try {
    ...
    if (adjustedAmount.includes("e")) {
      ...
      adjustedAmount = amountNum.toLocaleString("fullwide", {
        useGrouping: false,
      });
    ...
    const timeoutId = setTimeout(
      () => controller.abort(),
      apiConfig.config.timeout || 12000
    );
    ...
    console.log("KyberSwapRouteResponse", data);
    ...
    // 计算预估gas费用
    const gasSpent = routeSummary.gas
      ? Number.parseInt(routeSummary.gas, 10)
      : 150000;
    ...
    console.log(
      `KyberSwap quote successful: ${swapPrice} (${routeSummary.amountIn} -> ${routeSummary.amountOut})`
    );
    return quote;
  } catch (error) {
...
const fetchQuoteByProvider = async (
  ...
  try {
    console.log(
      `Attempting to fetch quote from ${apiConfig.provider} for chain ${chainId}`
    );
    ...
    switch (apiConfig.provider) {
      ...
      case "paraswap":
        return await fetchParaSwapQuote(
          apiConfig,
          chainId,
          tokenInAddress,
          tokenOutAddress,
      ...
      case "kyberswap":
        return await fetchKyberSwapQuote(
          apiConfig,
          chainId,
          tokenInAddress,
          tokenOutAddress,
...
export const fetchPriceQuoteFromProvider = async (
  ...
  try {
    ...
    console.log(
      `Using ${tokenDecimals} decimals for token ${tokenInSymbol} (${tokenInAddress})`
    );

    // 根据代币的实际小数位数转换金额
    const amountInWei = await convertToWeiWithDecimals(
    ...
    if (quote) {
      // 获取输出代币的小数位数
      const outputTokenDecimals = await getTokenDecimals(
        chainId,
        tokenOutAddress
      );

      // 转换输出金额
      const decimalAmountOut = convertWeiToDecimal(
        quote.assumedAmountOut,
        outputTokenDecimals
      );

      console.log(
        `转换输出金额从 ${quote.assumedAmountOut} 到 ${decimalAmountOut}`
      );
...
export const fetchBestPriceQuote = async (
  ...
  try {
    ...
    if (preferredProviderId) {
      preferredConfig = availableConfigs.find(
    ...
    // 如果首选提供商失败，尝试其他提供商
    console.warn(
      `Preferred provider ${preferredConfig.id} failed, trying fallback providers`
    );
    ...
    for (const config of availableConfigs) {
      ...
      quote = await fetchQuoteByProvider(
        config,
        chainId,
        tokenInAddress,
        tokenOutAddress,
    ...
    const results = await Promise.all(
      availableConfigs.map(async (config) => {
        ...
        if (quote) {
          ...
          console.log(
            `${config.name}: 转换输出金额从 ${quote.assumedAmountOut} 到 ${decimalAmountOut}`
          );
...
async function convertToWeiWithDecimals(
  amount: number,
  chainId: number,
  tokenAddress: string
): Promise<string> {
  try {
    ...
    console.log(
      `Converting amount ${amount} to wei with ${decimals} decimals for token ${tokenAddress}`
    );
...
export const executeArbitrageQuery = async (
  ...
  try {
    ...
    const getPriceQuoteDirect = async (
      ...
      if (!chainId || !tokenInAddress || !tokenOutAddress) {
        throw new Error(
          `无效的链或代币配置: ${chainName}, ${tokenInSymbol}, ${tokenOutSymbol}`
        );
      }
      ...
      console.log(
        `${isSourceChain ? "源链" : "目标链"}交易金额: ${amountToUse}`
      );
      ...
      if (preferredApiProvider) {
        const preferred = availableConfigs.find(
          (c) => c.id === preferredApiProvider
        );
        if (preferred) {
          apiConfig = preferred;
        }
      ...
      if (!quote) {
        throw new Error(
          `无法获取价格报价: ${chainName}, ${tokenInSymbol} -> ${tokenOutSymbol}`
        );
      }
    ...
    if (sourceOutputAmount.includes("e")) {
      ...
      adjustedSourceOutputAmount = outputNum.toLocaleString("fullwide", {
        useGrouping: false,
      });
      console.log(
        `转换输出金额从 ${sourceOutputAmount} 到 ${adjustedSourceOutputAmount}`
      );
    }
    ...
    // 获取源链目标代币（targetToken）地址
    const sourceChainId = getChainId(sourceChain);
    const targetTokenAddress = getTokenAddress(sourceChain, targetToken);

    // 获取目标链源代币（sourceToken）地址
    const targetChainId = getChainId(targetChain);
    const sourceTokenAddress = getTokenAddress(targetChain, sourceToken);

    // 在sourceQuote.tokens中查找与targetTokenAddress匹配的代币获取decimals
    let sourceOutputTokenDecimals = 18; // 默认值
    for (const token of sourceQuote.tokens) {
      if (token.address.toLowerCase() === targetTokenAddress?.toLowerCase()) {
        sourceOutputTokenDecimals = token.decimals;
        break;
      }
    }

    // 在targetQuote.tokens中查找与sourceTokenAddress匹配的代币获取decimals
    let finalOutputTokenDecimals = 18; // 默认值
    for (const token of targetQuote.tokens) {
      if (token.address.toLowerCase() === sourceTokenAddress?.toLowerCase()) {
        finalOutputTokenDecimals = token.decimals;
        break;
      }
    }

    console.log(`源链输出代币小数位: ${sourceOutputTokenDecimals}`);
    console.log(`目标链输出代币小数位: ${finalOutputTokenDecimals}`);

    // 使用正确的小数位数转换金额
    const decimalSourceOutputAmount = convertWeiToDecimal(
      sourceOutputAmount,
      sourceOutputTokenDecimals
    );

    const decimalFinalOutputAmount = targetQuote.amountOutUsd;

    console.log(
      `转换后的源链输出金额: ${decimalSourceOutputAmount} ${targetToken}`
    );
    console.log(
      `转换后的最终输出金额: ${decimalFinalOutputAmount} ${sourceToken}`
    );
...
export const getTokenDecimals = async (
  ...
  if (tokenDecimalsCache[cacheKey] !== undefined) {
    console.log(
      `Using cached decimals for token ${tokenAddress}: ${tokenDecimalsCache[cacheKey]}`
    );
  ...
  console.warn(
    `无法获取代币 ${tokenAddress} 在链 ${chainId} 的小数位数，使用默认值18`
  );
  return 18;
};
...
export const fetchTokenDecimals = async (
  ...
  try {
    ...
    console.error(
      `Error fetching token decimals for ${tokenAddress} on chain ${chainId}:`,
      error
    );
    return null;
  }
