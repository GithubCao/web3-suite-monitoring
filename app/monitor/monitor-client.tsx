...
// 模拟钱包管理器函数（避免导入错误）
const getWallets = () => []
const executeWalletTransaction = async () => ({ status: "failed", error: "钱包功能暂不可用" })
...
export default function MonitorPageClient() {
  ...
  }, [mounted, isMonitoring])

  // 监控价格
  useEffect(() => {
    if (!mounted || activeStrategies.length === 0) return
  ...
  }, [mounted, activeStrategies, updateInterval])
  ...
  const checkPrices = async () => {
    ...
    try {
      ...
      for (const strategy of activeStrategies) {
        try {
          // 安全检查策略属性
          if (!strategy?.amount || !strategy?.sourceChain || !strategy?.targetChain) {
            console.warn(`Strategy ${strategy?.id || "unknown"} has missing required properties`)
            continue
          }
          ...
          console.error(`Error checking prices for strategy ${strategy?.name || "unknown"}:`, error)
        }
      }
  ...
  const getWalletStatus = (walletAddress?: string) => {
    ...
    return <Badge variant="outline">钱包功能开发中</Badge>
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <TradeExecutionDialog
        open={tradeExecutionDialog.open && !!tradeExecutionDialog.strategy}
      ...
      <AutoTradeDialog
        open={autoTradeDialog.open && !!autoTradeDialog.strategy}
