...
import { getChainNames, getTokensForChain, getCachedTokensForChain, getCachedTokenBySymbol, getCachedTokenByAddress } from "@/lib/config"
...
export default function EditStrategyPage({ params }: { params: { id: string } }) {
  ...
  // 加载链上的代币详情
  const loadTokenDetails = useCallback(async (chainName: string, isSourceChain: boolean) => {
    try {
  ...
  // 懒加载代币详情
  const lazyLoadTokenDetails = useCallback((chainName: string, isSourceChain: boolean) => {
    // 如果已经在加载中，则不重复加载
  ...
  useEffect(() => {
    ...
    const updatedStrategy = {
      ...strategy,
      sourceTargetToken: strategy.sourceTargetToken || (sourceTokensList.length > 1 ? sourceTokensList[1] : sourceTokensList[0]),
      targetSourceToken: strategy.targetSourceToken || (targetTokensList.length > 1 ? targetTokensList[1] : targetTokensList[0])
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedStrategy: Strategy = {
      ...
      minProfitPercentage: parseFloat(String(formData.minProfitPercentage)) || 1.0,
