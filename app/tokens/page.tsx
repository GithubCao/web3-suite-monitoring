...
export default function TokensPage() {
  ...
  const handleAddToken = () => {
    ...
    try {
      ...
      // 显示成功消息
      setMessage(`成功添加代币: ${tokenToAdd.symbol}`);
      setMessageType('success');
    } catch (error) {
  ...
  const handleDeleteToken = () => {
    ...
    try {
      ...
      // 显示成功消息
      setMessage(`成功删除代币: ${selectedToken.symbol}`);
      setMessageType('success');
    } catch (error) {
  ...
  const handleDeleteChain = () => {
    ...
    try {
      ...
      const updatedChains = chains.filter(chain => 
        chain.chainId !== selectedChainToDelete.chainId
      );
          ...
          <div className="bg-white rounded-lg shadow p-6">
                ...
                <Dialog open={newTokenDialogOpen} onOpenChange={setNewTokenDialogOpen}>
                  ...
                  <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        为当前选择的网络 ({getSelectedChainName()}) 添加新代币
                      </DialogDescription>
                    </DialogHeader>
            ...
            <div className="mb-2 flex justify-between items-center">
              <h3 className="text-lg font-semibold">
                {getSelectedChainName()} 代币列表
              </h3>
              <span className="text-sm text-gray-500">
                共 {filteredTokens.length} 个代币
              </span>
            </div>
            ...
            {filteredTokens.length > 0 && (
              <div className="mt-4 flex justify-between items-center">
                <div className="text-sm text-gray-500">
                  显示 {(currentPage - 1) * tokensPerPage + 1} - {Math.min(currentPage * tokensPerPage, filteredTokens.length)} 共 {filteredTokens.length} 个代币
                </div>
                      ...
                      <div className="mt-4 flex justify-end">
                        <Button 
                          ...
                          onClick={() => {
                            ...
                            // 切换到代币选项卡
                            document.querySelector('[data-value="tokens"]')?.dispatchEvent(
